/*
 ...
 10/07/15: For T3 setup swapped Bed & Call pins over. LK1,LK2&LK3 set to 1-2 & LK4 to 2-3.
 23/01/2017: Added LOW BATTERY WARNING
 30/05/2017: fixed T3/T1 issue.
 */
...
const int Bed = 3;                          // PB3 - Pin2 BedLight    T1
const int Call = 4;                         // PB4 - Pin3 Call        T1
//const int Bed = 4;                          // PB4 - Pin3 BedLight      T3
//const int Call = 3;                         // PB3 - Pin2 Call          T3

//int unsigned myIDnum;                       // Initial ID number
int unsigned rfIDnum;                       // Received ID number
int unsigned LB_rfIDnum;                    // Received ID number with LOW BATTERY
char CharMsg[6];                            // RF Transmission container
boolean whichButton;                        // Call (true) or Bed light (false) press from Tx
char BS;                                    // BATERRY STATUS
...
void setup() {
  ...
  if(EEPROM.read(20) != 0x0D)
    EEPROM.write(20, 0x0D);                        //F/w Version - 1.3 x 10
  if(EEPROM.read(21) != 0x1E)
    EEPROM.write(21, 0x1E);                       // F/w date - 18 day
  if(EEPROM.read(22) != 0x05)
    EEPROM.write(22, 0x05);                       // F/w date - 01 month
...
void checkProg() {                          // Check Program button press
  ...
  if (current == LOW && previous == HIGH && count == 5) {   // Program Mode - Stores rfIDnum RF ID number transmitted.
    ...
    if (vw_get_message(buf, &buflen)) {   // Non-blocking
      ...
      BS = CharMsg[buflen - 1];     // Fill BATTERY STATUSE Char with corresponding chars from CharMsg
      CharMsg[buflen] = '\0';
...
void checkRf() {                            // Read Rf input buffer
  ...
  if (vw_get_message(buf, &buflen)) {       // Non-blocking
    ...
    BS = CharMsg[buflen - 1];     // BATTERY STATUSE
    ...
    if ((rfIDnum >= 0 && rfIDnum <= 16383) || (rfIDnum >= 32768 && rfIDnum <= 65533)) {  // Check if its a Call, Mat or AUX number
    ...
    if (( BS == 'L')&& (rfIDnum == id1 ||
      ...
      rfIDnum == id5 ))
    {
      LB_rfIDnum = rfIDnum;            // save ID with LOW battery
      LOW_BAT = true;
    }
    if ( (BS == 'H') && (rfIDnum == LB_rfIDnum))
      LOW_BAT = false;
    delay(500);                               // Wait so there is only one trigger.
  }
  if (LOW_BAT)
    ledBlinkhalf(5, 100, LED);
